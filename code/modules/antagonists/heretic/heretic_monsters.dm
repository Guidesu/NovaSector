...
/datum/antagonist/heretic_monster/on_removal()
	if(!silent)
		...
		if(owner.current)
			...
			owner.current.visible_message(span_deconversion_message("[owner.current] looks like [owner.current.p_theyve()] been freed from the chains of the Psyonics!"), ignored_mobs = owner.current)
...
/datum/antagonist/heretic_monster/proc/set_owner(datum/mind/master)
	...
	to_chat(owner, span_boldnotice("You are a [ishuman(owner.current) ? "shambling corpse returned":"horrible creation brought"] to this plane through the Gates of the Psyonics."))
