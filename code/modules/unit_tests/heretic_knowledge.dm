...
/datum/unit_test/heretic_knowledge/Run()
	...
	while(i < length(list_to_check))
		...
		if(!ispath(path_to_create))
			TEST_FAIL("Psyonaut Knowledge: Got a non-heretic knowledge datum (Got: [path_to_create]) in the list knowledges!")
		...
		for(var/datum/heretic_knowledge/next_knowledge as anything in instantiated_knowledge.next_knowledge)
			if(!ispath(next_knowledge))
				TEST_FAIL("Psyonaut Knowledge: [next_knowledge.type] has a [isnull(next_knowledge) ? "null":"invalid path"] in its next_knowledge list!")
				continue
	...
	if(length(all_possible_knowledge) != length(all_possible_knowledge & list_to_check))
		...
		for(var/datum/heretic_knowledge/lost_knowledge as anything in unreachables)
			TEST_FAIL("Psyonaut Knowledge: [lost_knowledge] is unreachable by players! Add it to another knowledge's 'next_knowledge' list. If it is purposeful, set its route to 'null'.")
...
/datum/unit_test/heretic_main_paths/Run()
	...
	for(var/datum/heretic_knowledge/knowledge as anything in subtypesof(/datum/heretic_knowledge))
		...
		if(isnull(paths[knowledge_route]))
			TEST_FAIL("Psyonaut Knowledge: An invalid knowledge route ([knowledge_route]) was found on [knowledge].")
			continue
	...
	for(var/main_path in paths)
		for(var/other_main_path in (paths - main_path))
			TEST_ASSERT(paths[main_path] == paths[other_main_path], \
				"Psyonaut Knowledge: [main_path] had [paths[main_path]] knowledges, \
