...
/datum/unit_test/heretic_rituals/Run()
	...
	for(var/datum/heretic_knowledge/knowledge as anything in all_ritual_knowledge)
		...
		if(!our_rune.do_ritual(our_heretic, knowledge))
			...
			// Aaand throw a fail.
			TEST_FAIL("Psyonaut rituals: ([knowledge.type]) Despite having all required atoms present, the ritual failed to transmute.")
			continue
		...
		for(var/result_item_path in knowledge.result_atoms)
			...
			if(!result)
				TEST_FAIL("Psyonaut rituals: ([knowledge.type]) Despite successfully completing the ritual, a resulting atom could not be found ([result_item_path])")
				continue
		...
		for(var/atom/thing as anything in nearby_atoms)
			...
			TEST_FAIL("Psyonaut rituals: ([knowledge.type]) After completing the ritual, there were non-result atoms remaining on the rune. ([thing] - [thing.type])")
			nearby_atoms -= thing
			qdel(thing)
