...
/obj/machinery/computer/auxiliary_base/proc/set_landing_zone(turf/T, mob/user, no_restrictions)
	...
	if(!base_dock) //Not all maps have an Aux base. This object is useless in that case.
		to_chat(user, span_warning("This station is not equipped with an auxiliary base. Please contact your Free Union of Vulken contractor."))
		return
	if(!no_restrictions)
...
/obj/structure/mining_shuttle_beacon/attack_hand(mob/user, list/modifiers)
	...
	for(var/S in SSshuttle.stationary_docking_ports)
		...
		if(SM.shuttle_id == "mining_home" || SM.shuttle_id == "mining_away")
			...
			break
	if(!Mport)
		to_chat(user, span_warning("This station is not equipped with an appropriate mining shuttle. Please contact Free Union of Vulken Support."))
		return
	...
	if(!mining_shuttle) //Not having a mining shuttle is a map issue
		to_chat(user, span_warning("No mining shuttle signal detected. Please contact Free Union of Vulken Support."))
