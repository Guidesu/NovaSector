...
/datum/controller/subsystem/dynamic/proc/send_intercept()
	...
	. = "<b><i>Free Union of Vulken Department of Intelligence Threat Advisory, Spinward Sector, TCD [time2text(world.realtime, "DDD, MMM DD")], [CURRENT_STATION_YEAR]:</i></b><hr>"
...
/datum/controller/subsystem/dynamic/proc/generate_advisory_level()
	...
	if (prob(PULSAR_REPORT_CHANCE))
		...
		advisory_string += "Your sector's advisory level is Pulsar Star. A large unknown electromagnetic field has stormed through nearby surveillance equipment. No surveillance data has been able to be obtained showing no credible threats to Free Union of Vulken assets within the Spinward Sector. The Department advises maintaining high alert against potential threats, regardless of a lack of information."
		return advisory_string
	...
	switch(round(shown_threat))
		...
		if(1 to 19)
			...
			if (show_core_territory)
				...
				advisory_string += "Your sector's advisory level is Green Star. Surveillance information shows no credible threats to Free Union of Vulken assets within the Spinward Sector at this time. As always, the Department advises maintaining vigilance against potential threats, regardless of a lack of known threats."
		if(20 to 39)
		...
		if(66 to 79)
			...
			advisory_string += "Your sector's advisory level is Red Star. The Department of Intelligence has decrypted Cybersun communications suggesting a high likelihood of attacks on Free Union of Vulken assets within the Spinward Sector. Stations in the region are advised to remain highly vigilant for signs of enemy activity and to be on high alert."
		if(80 to 99)
