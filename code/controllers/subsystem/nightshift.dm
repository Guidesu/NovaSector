	...
	if(high_security_mode != emergency)
		...
		if(night_time)
			...
			if(!emergency)
				...
				announce("Disabling night lighting: Colony is in a state of emergency.")
	if(emergency)
		night_time = FALSE
...
/datum/controller/subsystem/nightshift/proc/update_nightshift(active, announce = TRUE, resumed = FALSE, forced = FALSE)
	if(!resumed)
		...
		if(announce)
			if (active)
				announce("Good evening, colony! To reduce power consumption and stimulate the circadian rhythms of some species which is  to be expected, all of the light on your beloved colony have been automatically dimmed for the night.")
			else
				announce("Good morning, colony! As it is now day time, all of the lights aboard the colony have been restored to their former brightness.")
