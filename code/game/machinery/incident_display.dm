...
/obj/machinery/incident_display/examine(mob/user)
	...
	if(sign_features & DISPLAY_DELAM)
		if(last_delam >= 0)
			. += span_info("It has been [last_delam] shift\s since the last delamination event at this Free Union Of Vulken facility.")
			switch(last_delam)
				if(0)
	...
	if(sign_features & DISPLAY_TRAM)
		...
		switch(hit_count)
			...
			if(2 to 5)
				. += span_info("There's room for improvement.<br/>")
			if(6 to 10)
				. += span_info("Good work! Free Union Of Vulken's finest!<br/>")
			if(69)
				. += span_info("Nice.<br/>")
			else
