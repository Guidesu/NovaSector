...
/datum/controller/subsystem/lorecaster/Initialize()
	GLOB.news_network.create_feed_channel("Free Union of Vulken News Network", "NNN", "Get the latest stories from the frontier, here! For the not-quite-latest stories, download the \"News Archive\" app to any NTOS-based device today!", locked = TRUE, hardset_channel = 1001)
...
/datum/controller/subsystem/lorecaster/fire(resumed)
	...
	GLOB.news_network.submit_article(text || "Someone forgot to fill out the article!", title || "Free Union of Vulken News Broadcast", "Free Union of Vulken News Network", null)
